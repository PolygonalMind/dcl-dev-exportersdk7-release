...
public class SceneToGlTFWiz : MonoBehaviour
{
	...
	public void ExportCoroutine(string path, Preset presetAsset, bool buildZip, bool exportPBRMaterials, bool exportAnimation = true)
	{
		StartCoroutine(Export(path, presetAsset, buildZip, exportPBRMaterials, exportAnimation));
	}
	...
	public IEnumerator Export(string path, Preset presetAsset, bool buildZip, bool exportPBRMaterials, bool exportAnimation = true)
	{
       ...
       savedPath = Path.GetDirectoryName(path);
		...
		// Collect objects in the scene, add to lists
			...
			if (m != null)
			{
				...
				for (var i = 0; i < smCount; ++i)
				{
					...
					if (i < sm.Length) {
						...
						{
							primitive.materialIndex = GlTF_Writer.materialNames.IndexOf(matName);
						}
						else
						{
							...
							{
								material.instanceTechniqueIndex = GlTF_Writer.techniqueNames.IndexOf(techName);
							}
							else
							{
		...
		Debug.Log("Export has been done in " + path);
		if(buildZip)
		{
    ...
    public bool ExportGameObjectAndChildren(GameObject go, string path, Preset presetAsset, bool buildZip, bool exportPBRMaterials, bool exportAnimation = true, bool doConvertImages = false)
    {
        ...
        foreach (Transform tr in trs)
        {
            ...
            foreach (Transform t in tr.transform)
            {
                if (t.gameObject.activeInHierarchy && t != go.transform)
                    node.childrenNames.Add(GlTF_Node.GetNameFromObject(t));
            }
        ...
        if (GlTF_Writer.meshes.Count == 0)
        {
            Debug.LogError("No visible objects have been exported. Aborting export. Check the mesh from the GameObject: " + go);
            return false;
        }
	...
	{
		if ((mat.HasProperty("_Surface") && mat.HasProperty("_AlphaClip") && mat.GetFloat("_Surface") != 0) || (mat.IsKeywordEnabled("_ALPHATEST_ON") && mat.HasProperty("_AlphaClip")))
		{
			string mode = (mat.GetFloat("_Blend") == 0 && mat.GetFloat("_AlphaClip") == 1) ? "MASK" : "BLEND";
			...
			//Debug.Log("Transparency is True");
	...
	private void unityToPBRMaterial(Material mat, ref GlTF_Material material)
	{
		...
		{
			Debug.LogError("The material: \"" + mat.name + "\" has a non supported shader: " + mat.shader + "\nYou should use Polyshader instead");
			isMaterialPBR = false;
			if (EditorUtility.DisplayDialog("WARNING", "Material \"" + mat.name + "\" is NOT suported!!\nYou should use POLYSHADER instead of \n" + mat.shader, "Ok"))
			{
		...
		else
		{
			...
			isMetal = mat.shader.name == "Universal Render Pipeline/Lit";
		...
		//Parse diffuse channel texture and color _MainTex
		if (mat.HasProperty("_BaseMap") && mat.GetTexture("_BaseMap") != null)
		{
			var textureValue = new GlTF_Material.DictValue();
			textureValue.name = isMetal ? "baseColorTexture" : "diffuseTexture";

			int diffuseTextureIndex = processTexture((Texture2D)mat.GetTexture("_BaseMap"), hasTransparency ? IMAGETYPE.RGBA : IMAGETYPE.RGBA_OPAQUE);
		...
		{
			emissiveFactor.color = mat.GetColor("_EmissionColor") * 1 / 3; //Set intensity lower to match Emissive applyed by Dcl
		}
        ...
        if(mat.HasProperty("_Cull") && mat.GetFloat("_Cull") == 2)			// 2 = Front Face
        {
            doubleSided.value = false;

        }
		else if (mat.HasProperty("_Cull") && mat.GetFloat("_Cull") == 1)    // 1 = Back Face
		{
			doubleSided.value = false;
			Debug.LogWarning(mat.name.ToString() + " Forced to render front face. Can't render only back faces.");
		}
		else                                                                // 0 = Both Faces
	...
	public string convertTexture(ref Texture2D inputTexture, string pathInProject, string exportDirectory, IMAGETYPE format)
	{
        ...
        string pathInArchive = "gltfTextures";
        string exportDir = Path.Combine(exportDirectory, pathInArchive);

		if (!Directory.Exists(exportDir))
			Directory.CreateDirectory(exportDir);
		
        string outputFilename = Path.GetFileNameWithoutExtension(pathInProject) + ((format == IMAGETYPE.RGBA || format == IMAGETYPE.RGBA_OPAQUE) ? ".png" : ".jpg");
        string exportPath = exportDir + "/" + outputFilename;  // relative path inside the .zip
		string pathInGltfFile = pathInArchive + "/" + outputFilename;
		
        File.WriteAllBytes(exportPath, (format == IMAGETYPE.RGBA || format == IMAGETYPE.RGBA_OPAQUE) ? newtex.EncodeToPNG() : newtex.EncodeToJPG( format== IMAGETYPE.NORMAL_MAP ? 95 : jpgQuality));
